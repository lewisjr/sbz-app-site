<script lang="ts">
	...
	const openAccount = async () => {
		...
		form.append(`aco`, acoValue ? acoValue : "");
		...
		try {
			const req = await fetch("/api/admin/_su", {
				method: "POST",
				body: form,
			});
	...
	const getAccount = async () => {
		...
		try {
			const req = await fetch(`/api/admin/onboard/v2?user=${cvNum}`, {
				method: "GET",
			});
			...
			const res: {
				success: boolean;
				message: string;
				data: SBZdb["public"]["Tables"]["clients"]["Row"] | undefined;
			} = await req.json();
			...
			if (!res.success) {
				...
				if (!res.data) {
					toast.success("Please proceed to onboard the client or double check the CV number.");
				} else {
					const {
						acc_type,
						approve_date,
					...
					// set active tab
					if (acc_type === "individual" && !is_in_trust_of) activeTab = "individual";
					if (acc_type === "individual" && is_in_trust_of) {
						activeTab = "individual";
						isInTrustOf = "yes";
					}
					if (acc_type === "joint") activeTab = "joint";
					if (acc_type === "institution") activeTab = "institution";

					luseId = luse_id.toString();
					...
					switch (acc_type) {
						...
						case "institution":
							break;
						default:
							toast.error("Inspect the account type.");
							return;
					}
				}
			}
...
<div class="main-tainer">
	...
	<div class="tainer">
		...
		{:else}
			...
			<Tabs.Root bind:value={activeTab} class="mx-auto mb-5" disabled>
				<Tabs.List>
					<Tabs.Trigger class="cursor-pointer" value={activeTab}
						>{toTitleCase(activeTab)}</Tabs.Trigger
					>
				</Tabs.List>
			</Tabs.Root>
				...
				<div class="items tp flex">
					<div class="cntnt-l flex w-full max-w-sm flex-col gap-1.5">
						...
						<Input
							class="my-2"
							bind:value={cvNum}
							placeholder="CV54321AB"
							disabled={isOpen ?? loading}
							oninput={(e) => {
								//@ts-ignore
								cvNum = e.target.value.trim().toUpperCase();
							}}
							onkeypress={(e) => {
								if (e.key === "Enter") getAccount();
							}}
							inputmode="text"
						/>
						<Button onclick={() => (isOpen = false)}>Reset Form</Button>
					</div>
				</div>
			</section>
			...
			{#if isOpen}
						...
						<section class="inputs">
							...
							<div class="items tp mt-7 flex">
								<div class="cntnt-l flex w-full max-w-sm flex-col gap-1.5">
									...
									<p class="mb-4 text-justify text-sm text-muted-foreground">
										Upload a tenancy agreement, tax certificate, utility bill, or bank statement
										from the past three months.
									</p>
									<p class="text-justify text-sm text-muted-foreground">
										<b>Ensure</b> they are certified by the police, court, church, or commisioner of
										oaths, and that they cleary show your official names and address.
									</p>
								</div>
							</div>
						</section>
				...
				{#if activeTab === "joint"}
					...
					<section class="inputs">
						...
						{:else}
							<p class="tmid">
								No partners added. Fill in the form above to being adding partners.
							</p>
						{/if}
					</section>
				...
				{#if activeTab === "institution"}
								...
								<Table.Body>
									{#each directors as director, index}
										<Table.Row>
											...
											<Table.Cell
												><Button onclick={() => deleteDirector(index)}><Trash2 /></Button
												></Table.Cell
											>
										</Table.Row>
									{/each}
					...
					<section class="inputs">
								...
								<Table.Body>
									{#each instituteManagers as manager, index}
										<Table.Row>
											...
											<Table.Cell
												><Button onclick={() => deleteManager(index)}><Trash2 /></Button
												></Table.Cell
											>
										</Table.Row>
									{/each}
						...
						{:else}
							<p class="tmid">
								No managers added. Fill in the form above to being adding managers.
							</p>
						{/if}
					</section>
			...
			-->
				...
				<section class="inputs mb-5">
					<div class="items tp mt-7 flex">
						<div class="cntnt-l flex w-full max-w-sm flex-col gap-1.5">
							<Label class="mb-1">Proof of Identity</Label>
							<Input type="file" onchange={handleAcoUpload} accept=".pdf" />
							<p class="mb-4 text-justify text-sm text-muted-foreground">
								Upload the submitted account opening document.
							</p>
						</div>
					</div>
				</section>
