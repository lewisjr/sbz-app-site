<script lang="ts">
	...
	import Scanify from "$lib/components/Scanify.svelte";
	...
	//types
	import type { SBZdb, Types, GenericResponseWData } from "$lib/types";
	...
	let kycDetails = $state<Types["ClientKyc"][]>([]);

	const checkUser = async () => {
		loading = true;
		try {
			const req = await fetch(`/api/admin/onboard?user=${luseId}`);
			const res: GenericResponseWData<{ user: Types["UserData"]; kyc: Types["ClientKyc"][] }> =
				await req.json();
			...
			checked = true;
			activeTab = res.data.user.acc_type;

			kycDetails = res.data.kyc;
		} catch (ex) {
			toast.error(String(ex));
			loading = false;
		}
...
<div class="main-tainer">
	<div class="tainer">
		<h2 class="mb-5 text-center">Onboard Client</h2>
		{#if checked}
			<h3 class="tmid my-2">Account Type</h3>

			<Tabs.Root bind:value={activeTab} class="mx-auto mb-5">
				<Tabs.List>
					<Tabs.Trigger class="cursor-not-allowed" value={activeTab}
		...
		<section class="inputs mb-5">
			...
			<Button
				disabled={luseId.length === 0 ? true : loading}
				onclick={() => checkUser()}
				class="mt-5"
				...
				<div class="items tp flex">
					<div class="flex w-full max-w-sm flex-col gap-1.5">
						...
						{#if kycDetails.length}
							<ul>
								{#each kycDetails as kyc}
									<li>{kyc.title}</li>
								{/each}
							</ul>
						{:else}
							<p>This client has no KYC on file.</p>
						{/if}
					</div>
				</div>
				<div class="items tp flex">
					<div class="flex w-full max-w-sm flex-col gap-1.5">
						<Button class="mt-5">Add KYC<PlusCircle class="ml-2 h-4 w-4" /></Button>
					</div>
				</div>
			</section>
		{/if}
