<script lang="ts">
	...
	let { key, stockData, symbol, isMobile = $bindable() }: Props = $props();
				...
				yaxis: {
					labels: {
						formatter: (val, ops) => {
							...
							else if (twoDecWhitelist.includes(key))
								return val > 999 ? numParse(val.toFixed(2)) : val.toFixed(2);
						...
						offsetX: 10 + 8 * _data[0].toString().length,
						offsetY: -10,
					},
					floating: true,
					/*
					axisBorder: {
						...
						offsetX: 10 + 11 * -1 * _data[0].toString().length,
					},
					*/
				},
				xaxis: {
