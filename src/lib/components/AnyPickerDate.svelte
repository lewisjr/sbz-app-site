<script lang="ts">
	...
	interface Props {
		...
		/**Should be the last element in the array*/
		end?: boolean;
	}

	let { data, handler, pickerTitle, value, end }: Props = $props();
	...
	$effect(() => {
		...
		let filter: "string" | "number" = "string";

		data.forEach((d, i) => {
			if (!i && typeof d.value === "number") filter = "number";
